AT.prototype.atFormHelpers = {
    ...
    showTitle: function(){
        ...
        if (state === 'verifyEmail' || (Meteor.user() && state === 'signIn'))
            return false;
        return !(state === 'signIn' && oauthServices().length);
    },
    ...
    showServicesSeparator: function(){
        ...
        return rightState && !Meteor.user() && pwdService && oauthServices().length;
    },
    showError: function() {
    ...
    showSignUpLink: function(){
        var state = this.state || AccountsTemplates.getState();
        return ((state === 'signIn' && !Meteor.user()) || state === 'forgotPwd') && !AccountsTemplates.getConfig('forbidClientAccountCreation');
    },
