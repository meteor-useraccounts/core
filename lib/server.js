...
AT.prototype.init = function() {
    ...
    // Cheks meteor version to log a warning for the new package name
    console.log(Meteor.release);
    console.log(Meteor.release.substr(0, 8));
    if (Meteor.release.substr(8) === "METEOR@1") {
        console.warn("This package is not supposed to be used with Meteor@1.x.x.");
        console.warn("Please install useraccounts:core and related packages instead!");
        console.warn("GitHub repository at: https://github.com/meteor-useraccounts/ui");
    }

    // Checks there is at least one account service installed
    if (!Package["accounts-password"] && (Accounts.oauth.serviceNames().length === 0))
