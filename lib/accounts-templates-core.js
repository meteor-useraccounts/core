...
if (Meteor.isClient) {
    ...
    var validateField = function(event) {
        ...
        Meteor.call('ATValidateField', fieldName, value, function(error, result) {
    ...
    AT.prototype.atSocialHelpers = {
        ...
        disabled: function() {
            ...
            //console.log(user && user.services && user.services[this]);
        ...
        'click #at-btn': function(event, t) {
            ...
            if (AccountsTemplates.getState() === 'sgup') {
                ...
                while (i--) {
                    ...
                    console.log('\t\tAT_field_' + fieldName);
                    sgupInfo[fieldName] = t.find('#AT_field_' + fieldName).value;
                }
                console.log('Signup info:');
                console.dir(sgupInfo);

                Meteor.call('ATSignup', sgupInfo, function(error, result) {
                    if (error) {
            ...
            // Sign In
            if (AccountsTemplates.getState() === 'sgin') {
                var login = t.find('#AT_field_login').value;
    ...
    AT.prototype.atSocialEvents = {
        'click button.btn': function(event, t) {
            event.preventDefault();
