...
AT.prototype.CONFIG_PAT = {
    ...
    // accounts-merge
    allowAccountsMerging: Match.Optional(Boolean),
    ...
    if (Meteor.isClient) {
        ...
        Meteor.call("ATEmailConfigured", function(err, result) {
            if (!err) {
                ...
                console.log('++ Email configured: ' + self.emailConfigured);
            }
...
if (Meteor.isClient) {
    ...
    AT.prototype.isDisabled = function(value) {
    ...
    AT.prototype.signinFormHelpers = {
        allowMerging: function() {
            return AccountsTemplates.ready() && AccountsTemplates.getConfig('allowAccountsMerging');
        },
        ...
        disabled: function() {
            if (AccountsTemplates.ready() && AccountsTemplates.isDisabled())
                return 'disabled';
            return '';
        },
    ...
    AT.prototype.atInputHelpers = {
        ...
        displayFormLabels: function() {
            return AccountsTemplates.ready() && AccountsTemplates.getConfig('displayFormLabels');
        },
        displayName: function() {
            var dName = this.displayName || this.name;
            //return T9n.get(dName, markIfMissing=false);
            var text = T9n.get(dName);
            if (text.substr(0, T9n.missingPrefix.length) == T9n.missingPrefix)
                return dName;
            return text;
        },
        errorText: function() {
            if (AccountsTemplates.ready()) {
                ...
                var errorLabel = AccountsTemplates.getFieldError(this.name);
                var errorText = T9n.get(errorLabel);
                if (errorText.substr(0, T9n.missingPrefix.length) == T9n.missingPrefix)
                    return errorLabel;
                return errorText;
            }
        },
    ...
    AT.prototype.atSocialHelpers = {
        ...
        show: function() {
            ...
            if (!(unconfigured && !showUnconfigured))
                return true;
        }
    };
    ...
    AT.prototype.atSocialEvents = {
        'click button': function(event, t) {
            ...
            event.preventDefault();
            console.dir(t);
            var serviceName = t.find('button').id.split('-')[1];
            var loginWithService = Meteor["loginWith" + capitalize(serviceName)];
            
            // FIXME: enable this once accounts-merge is stable and tested enough
            //var loginWithService = Meteor["signInWith" + capitalize(serviceName)];

            var callback = function(err, mergedUsers) {
                if (!err) {
...
if (Meteor.isClient) {
    ...
    Router.onStop(function() {
        console.log('Router stop at: ' + this.path);
        AccountsTemplates.clearFieldErrors();
        AccountsTemplates.setPrevPath(this.path);
    });
    ...
    console.log('Setting resetPassword route');
    Router.map(function() {
    ...
    Meteor.startup(function(){
        Router.map(function() {
            console.dir(AccountsTemplates._config);
            console.log('Email configured: ' + AccountsTemplates.emailConfigured);

            // Possibly sets up sign in route
            var signInRoutePath = AccountsTemplates.getConfig('signInRoutePath');
            console.log(signInRoutePath);
            if (signInRoutePath){
                console.log('Setting up signInRoute');
            ...
            // Possibly sets up sign in route
            var signUpRoutePath = AccountsTemplates.getConfig('signUpRoutePath');
            console.log(signUpRoutePath);
            if (signUpRoutePath){
                console.log('Setting up signUpRoute');
            ...
            var forgotPwdRoutePath = AccountsTemplates.getConfig('forgotPwdRoutePath');
            console.log(forgotPwdRoutePath);
            if (forgotPwdRoutePath){
                console.log('Setting up forgotPwdRoute');
