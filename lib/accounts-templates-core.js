...
AT.prototype.CONFIG_PAT = {
    ...
    // accounts-meld
    allowAccountsMelding: Match.Optional(Boolean),
...
AT.prototype.init = function() {
    ...
    if (Meteor.isServer) {
        ...
        // Possibly enables accounts merging
        if (this.getConfig('allowAccountsMerging')){
            console.log('Enabling accounts merging!!!');
            ...
            Accounts.onLogin(function(info) {
                ...
                updateEmails(info);
                // Checks for possible merge actions to be created
                checkForMerges(info.user);
            });
    ...
    if (Meteor.isClient) {
        ...
        if (this.getConfig('allowAccountsMerging')){
            console.log('Subscribing to register userRegisteredServices');
...
if (Meteor.isClient) {
    ...
    AT.prototype.ensureSignedIn = function(pause) {
        if (!Meteor.user()) {
            ...
            this.render('fullPageSigninForm');
            this.renderRegions();
    ...
    AT.prototype.signinFormHelpers = {
        allowMelding: function() {
            return AccountsTemplates.getConfig('allowAccountsMelding');
        },
    ...
    AT.prototype.atSocialHelpers = {
        ...
        show: function() {
            ...
            if (user && user.services && user.services[serviceName])
                // Don't show already registered services
                return false;
            var unconfigured = Accounts.loginServiceConfiguration.find({
                service: serviceName
            }).fetch().length === 0;
            var showUnconfigured = typeof Accounts._loginButtonsSession !== "undefined";
            return !(unconfigured && !showUnconfigured);
        }
    };
    ...
    AT.prototype.atSocialEvents = {
        'click button': function(event, t) {
            ...
            var callback = function(err, melddUsers) {
                AccountsTemplates.setDisabled(false);
                if (!err) {
                    // melddUsers is set if a meld occured
                    if (melddUsers) {
                        //console.log('melddUsers', melddUsers);

                        // TODO: Meteor.userId() can't be called here since we don't wait for the login to complete in Meteor.signInWithX, thus
                        // we have to pass along both sourceUserId and destinationUserId to this callback (melddUsers). It would be nicer if 
                        // 'melddUsers' was only the sourceUserId (instead of an object).

                        // The source account (melddUsers.sourceUserId) has now been deleted, so now is your chance to deal with you application specific
                        ...
                        /*
                    Meteor.call('meldItems', melddUsers.sourceUserId, melddUsers.destinationUserId, function(error, result) {
                        if (error) {
