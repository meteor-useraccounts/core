    ...
    if (Meteor.isClient) {
        ...
        Meteor.call("ATEmailConfigured", function(err, result) {
            if (!err){
                ...
                //console.log('emailConfigured: ' + self.emailConfigured);
            }
        });
    }
}
...
AT.prototype.init = function() {
    ...
    if (Meteor.isClient) {
        ...
        /*
        console.log('Setting up resetPassword route 1!!!');
        var self = this;
        Meteor.call("ATEmailConfigured", function(err, result) {
            if (!err)
                self.emailConfigured = result;
                ...
                if (result){
                    console.log('Setting up resetPassword route!!!');
        ...
        */
        ...
        Router.map(function() {
            /*
            console.log('Setting up resetPassword route 1!!!');
            if (self.emailConfigured){
                console.log('Setting up resetPassword route 2!!!');
                // FIXME: try to do this only if email is configured
                //        at the moment the problem is calling the method ATEmailConfigured to discover
                //        to set it up or not. The delay introduced by the method call seems to cause
                //        route set-up to be late: when the link on the email is clicked and the page loaded
...
AT.prototype.forbidClientAccountCreation = function () {
    return this.getConfig('forbidClientAccountCreation');
};
...
if (Meteor.isClient) {
    ...
    AT.prototype._isValidState = function(value) {
        if (this.forbidClientAccountCreation() && value === "sgup")
            return false;
    ...
    AT.prototype.signinFormHelpers = {
        ...
        loginServices: function() {
            return Accounts.oauth && Accounts.oauth.serviceNames();
        },
        ...
        showSignUpLink: function(){
            return AccountsTemplates.ready() && !AccountsTemplates.forbidClientAccountCreation();
        },
        signedInAs: function() {
        ...
        'submit #at-form-login': function(event, t) {
            ...
            if (AccountsTemplates.getState() === 'sgup') {
                ...
                _.map(fields, function(fieldName){
                    ...
                    if (!!value)
                        signupInfo[fieldName] = t.find('#AT_field_' + fieldName).value;
                });
            ...
            if (AccountsTemplates.getState() === 'fpwd') {
                ...
                var validationErr = AccountsTemplates.fullFieldValidation('login', login);
                if (validationErr) {
                ...
                return Accounts.forgotPassword({email: login}, function(error) {
                    if (error) {
                        AccountsTemplates.setFieldError('overall', error.reason);
