AT.prototype.atInputHelpers = {
    ...
    optionalText: function(){
        return "(" + T9n.get(AccountsTemplates.options.optionalField, markIfMissing=false) + ")";
    },
    errorText: function() {
    ...
    "focusout input": function(event){
        ...
        if (value && fieldId.substr(fieldId.length - 6) == "_again"){
            ...
            if (value !== $(pwd_input_id).val()){
                AccountsTemplates.state.fields.set(fieldId, AccountsTemplates.options.errorText.pwdMismatch);
                return;
            }
        }
