...
AT.prototype.atFormEvents = {
    ...
    'submit #at-form': function(event, t) {
        ...
        if (state === 'signUp') {
            ...
            var signupInfo = {};
            var fields = AccountsTemplates.getFieldIds();
            var visibleFields = [];
            _.map(fields, function(fieldId){
                var fieldInput = t.find('#AT_field_' + fieldId);
                if (fieldInput){
                    visibleFields.push(fieldId);
                    var value = fieldInput.value;
                    if (!!value)
                        signupInfo[fieldId] = value;
                }
            });
            ...
            var someError = false;
            var errList = [];
            _.each(visibleFields, function(fieldId){
                var value = signupInfo[fieldId] || '';
                validationErr = AccountsTemplates.validateField(fieldId, value, 'strict');
                if (validationErr) {
                    AccountsTemplates.setFieldError(fieldId, validationErr);
                    var fieldObj = AccountsTemplates.getField(fieldId);
                    if (!fieldObj.negativeValidation){
                        var fId = T9n.get(fieldObj.displayName, markIfMissing=false);
                ...
                if (error) {
                    ...
                    if(typeof(error.details) === 'object'){
                        ...
                        for (var fieldId in fieldErrors)
                            AccountsTemplates.setFieldError(fieldId, fieldErrors[fieldId]);
                    }
                    else{
        ...
        if (state === 'forgotPwd') {
            ...
            if (validationErr) {
                AccountsTemplates.setFieldError('email', validationErr);
                return;
            }
            AccountsTemplates.setDisabled(true);
        ...
        // Reset Password / Enrol Account
        if (state === 'resetPwd' || state === 'enrolAccount') {
            var paramToken = AccountsTemplates.paramToken;
            var pwd_field_id;
            if (state === 'resetPwd')
                pwd_field_id = 'new_password';
            else
                pwd_field_id = 'password';
            password = t.find('#AT_field_' + pwd_field_id).value;
            check(paramToken, String);
            check(password, String);
            // NOTE: This is the only place where password validation can be enforced!
            validationErr = AccountsTemplates.validateField(pwd_field_id, password, 'strict');
            if (validationErr) {
                AccountsTemplates.setFieldError(pwd_field_id, validationErr);
                AccountsTemplates.setDisabled(false);
                return;
            }
            if (AccountsTemplates.getConfig('confirmPassword')){
                password_again = t.find('#AT_field_' + pwd_field_id + '_again').value;
                // Checks password matching
                if (password !== password_again){
                    AccountsTemplates.setFieldError(pwd_field_id, T9n.get('error.pwdsDontMatch'));
                    AccountsTemplates.setFieldError(pwd_field_id + '_again', T9n.get('error.pwdsDontMatch'));
                    AccountsTemplates.setDisabled(false);
                    return;
                }
            }
