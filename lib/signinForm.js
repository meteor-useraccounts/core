...
AT.prototype.signinFormEvents = {
    ...
    'submit #at-form-login': function(event, t) {
        ...
        if (AccountsTemplates.getState() === 'rpwd' || AccountsTemplates.getState() === 'enro') {
            ...
            // NOTE: This is the only place where password validation can be enforced!
            var validationErr = AccountsTemplates.fullFieldValidation(pwd_field_name, password);
            if (validationErr) {
            ...
            if (AccountsTemplates.getConfig('confirmPassword')){
                var password_again = t.find('#AT_field_' + pwd_field_name + '_again').value;
                // Checks password matching
                if (password !== password_again){
        ...
        if (AccountsTemplates.getState() === 'cpwd') {
            ...
            check(password, String);
            // NOTE: This is the only place where password validation can be enforced!
            var validationErr = AccountsTemplates.fullFieldValidation('new_password', password);
            if (validationErr) {
            ...
            if (AccountsTemplates.getConfig('confirmPassword')){
                var password_again = t.find('#AT_field_new_password_again').value;
                // Checks password matching
                if (password !== password_again){
