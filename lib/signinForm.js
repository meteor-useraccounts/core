...
AT.prototype.signinFormHelpers = {
    ...
    showServicesSeparator: function(){
        var pwdService = !(_.isEmpty(Package['accounts-password']));
        var state = this.state || AccountsTemplates.getState();
        var sgin = state === 'sgin';
        return sgin && !Meteor.user() && pwdService && otherLoginServices();
    },
    showPwdService: function() {
        ...
        return !(_.isEmpty(Package['accounts-password'])) && !Meteor.user();
    },
    state: function(){
    ...
    showTermsLink: function(){
        ...
        return state === 'sgup' && (!!AccountsTemplates.getConfig('privacyUrl') || !!AccountsTemplates.getConfig('termsUrl'));
    },
    privacyUrl: function(){
