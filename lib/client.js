...
AT.prototype.STATES = [
    ...
    "enrolAccount", // Account Enrolment
...
// Token provided for routes like reset-password and enrol-account
...
AT.prototype._isValidState = function(value) {
    ...
    if (user && _.contains(['enrolAccount', 'forgotPwd', 'resetPwd', 'signUp'], value))
        return false;
...
AT.prototype.init = function() {
    ...
    var usernamePresent = _.contains(fieldIds, 'username');
    var emailPresent = _.contains(fieldIds, 'email');
    ...
    if (this._loginType === 'username_and_email'){
        ...
        if (!_.contains(fieldIds, 'username_and_email')){
    ...
    // it was not configured
    if (!_.contains(fieldIds, 'new_password')){
        fieldId = _.indexOf(fieldIds, 'password');
        var newPassword = _.clone(this._fields[fieldId]);
    ...
    if (this.getConfig('confirmPassword')){
        ...
        // it was not configured
        if (!_.contains(fieldIds, 'password_again')){
            fieldId = _.indexOf(fieldIds, 'password');
            var pwdAgain = _.clone(this._fields[fieldId]);
            pwdAgain._id = 'password_again';
            pwdAgain.displayName = "passwordAgain";
            this._fields.splice(fieldId + 1, 0, pwdAgain);
        }
        // Possibly updates field ids after automatic field insertions
        fieldIds = this.getFieldIds();
        ...
        // it was not configured
        if (!_.contains(fieldIds, 'new_password_again')){
            fieldId = _.indexOf(fieldIds, 'new_password');
            var newPasswordAgain = _.clone(this._fields[fieldId]);
            newPasswordAgain._id = 'new_password_again';
            newPasswordAgain.displayName = "newPasswordAgain";
            this._fields.splice(fieldId + 1, 0, newPasswordAgain);
        }
        // Possibly updates field ids after automatic field insertions
        fieldIds = this.getFieldIds();
    }
    ...
    if (this.getConfig('enablePasswordChange')){
        if (!_.contains(fieldIds, 'current_password')){
            this._fields.splice(0, 0, {
    ...
    _.each(this._fields, function(field){
        ...
        switch(field._id) {
            ...
            case 'new_password_again':
                ...
                break;
            case 'password':
                field.visible = ['enrolAccount', 'signIn', 'signUp'];
                break;
            case 'password_again':
                field.visible = ['enrolAccount', 'signUp'];
                break;
            case 'username':
