...
AT.prototype.getErrorText = function(error){
...
AT.prototype.submitCallback = function(error, state, onSuccess){
    if (error) {
        if(_.isObject(error.details))
            ...
            AccountsTemplates.setFieldError('overall', [AccountsTemplates.getErrorText(error)]);
    }
    else{
        if (onSuccess)
        onSuccess();
        if (_.contains(["changePwd", "enrollAccount", "forgotPwd"], state)){
            var redirectTimeout = AccountsTemplates.getConfig('redirectTimeout');
