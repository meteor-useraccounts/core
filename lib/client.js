...
AT.prototype.ensureSignedIn = function() {
    if (!Meteor.user()) {
        AccountsTemplates.setPrevPath(Router.current().route.path());
...
AT.prototype.init = function() {
    ...
    */
    ...
    var moveFieldAfterPwd = function(field_name) {
        var fieldIds = AccountsTemplates.getFieldIds();
        console.log("\nfields pre: " + fieldIds);
        var passwordId = _.indexOf(fieldIds, "password");
        var fieldId = _.indexOf(fieldIds, field_name);
        if (fieldId !== -1 && fieldId < passwordId){
            console.log("moving " + field_name);
            // removes the field
            var field = AccountsTemplates._fields.splice(fieldId, 1)[0];
            // push the field right after password
            AccountsTemplates._fields.splice(passwordId, 0, field);
        }
        console.log("fields post: " + AccountsTemplates.getFieldIds());
    };
    ...
    moveFieldAfterPwd("new_password_again");
    moveFieldAfterPwd("new_password");
    moveFieldAfterPwd("current_password");
    moveFieldAfterPwd("password_again");
    moveFieldAfterPwd("password");

    this._fields = _.map(this._fields, function(field){
        return new Field(field);
    });
