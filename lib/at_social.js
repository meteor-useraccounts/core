AT.prototype.atSocialHelpers = {
    ...
    buttonText: function() {
        ...
        if (!service.configured)
            return T9n.get(AccountsTemplates.texts.socialConfigure) + " " + serviceName;
        ...
        if (user && showAddRemove){
            if (user.services && user.services[this._id]){
                ...
                if (numServices === 2)
                    ...
                    return T9n.get(AccountsTemplates.texts.socialRemove) + " " + serviceName;
            } else
                    return T9n.get(AccountsTemplates.texts.socialAdd) + " " + serviceName;
        }
        var parentData = Template.parentData(1);
        var state = (parentData && parentData.state) || AccountsTemplates.getState();
        var prefix = state === "signIn" ?
            T9n.get(AccountsTemplates.texts.socialSignIn) :
            T9n.get(AccountsTemplates.texts.socialSignUp); 
        return prefix + " " + T9n.get(AccountsTemplates.texts.socialWith) + " " + serviceName;
    },
};
