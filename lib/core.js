...
AT.prototype.configureButtonText = function(buttonText) {
    check(buttonText, STATE_PAT);
    // Configuration options can be set only before initialization
    if (this._initialized)
        throw new Error("Button Text Configuration options must be set before AccountsTemplates.init!");

    if (Meteor.isClient)
        // Updates the current buttonTexts object
        this.buttonText = _.defaults(buttonText, this.buttonText);
}
...
AT.prototype.configureTitle = function(title) {
    check(title, STATE_PAT);
    // Configuration options can be set only before initialization
    if (this._initialized)
        throw new Error("Title Configuration options must be set before AccountsTemplates.init!");

    if (Meteor.isClient)
        // Updates the current titles object
        this.title = _.defaults(title, this.title);
}
