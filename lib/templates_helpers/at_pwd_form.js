...
AT.prototype.atPwdFormEvents = {
    ...
    "submit #at-pwd-form": function(event, t) {
        ...
        if (state === "signIn") {
            ...
            // Call preSignInHook, if any...
            var preSignInHook = AccountsTemplates.options.preSignInHook;
            if (preSignInHook) {
                // async call to let the user make a meteor.call inside his Hook
                preSignInHook(email, password, function(err, result) {
                    if (err) {
                        return AccountsTemplates.submitCallback(err, state);
                    } else {
                ...
                // If the hook is not defined continue the process
