...
* `allowAccountsMerging` - (Boolean, default true) Tells whether to use [accounts-merge](https://atmospherejs.com/package/accounts-merge) **WIP** still tentative, many issues to be solved, **please do not use unless you know what you're doing**.
...
* `signInRoutePath` - (optional String, default none) Specifies the path for the sign-in action. **Note: without a configured signInRoutePath `AccountsTemplates.ensureSignedIn` won't work!**
