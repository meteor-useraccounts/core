...
| preSignInHook               | Function |           | `func(email, password)` Called just before submitting the `pwdForm` for sign-in: allows for custom actions on the data being submitted. A nice use could be another user/pass validating method. The plain text `email` and `password` are provided for any reasonable use. |
