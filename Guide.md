...
| preSignInHook               | Function |           | `func(email, password)` Called just before submitting the `pwdForm` for sign-in: allows for custom actions on the data being submitted. A nice use could be to Meteor.call() to get data from server before validating the logIn. to be taken on form submission. The plain text `password` and `email` is also provided for any reasonable use. |
