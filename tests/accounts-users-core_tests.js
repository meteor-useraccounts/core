...
Tinytest.add("AccountsUsers - addField/removeField", function(test) {
    if (Meteor.isClient) {
        // addField cannot be called client-side
        test.throws(function() {
            AccountsUsers.addField('');
        }, function(err) {
            if (err instanceof Meteor.Error && err.details === 'AccountsUsers.addField can be called server-side only!')
                return true;
        });
        // removeField cannot be called client-side
        test.throws(function() {
            AccountsUsers.removeField('');
        }, function(err) {
            if (err instanceof Meteor.Error && err.details === 'AccountsUsers.removeField can be called server-side only!')
                return true;
        });
    } else {
...
Tinytest.add("AccountsUsers - addFields", function(test) {
    if (Meteor.isClient) {
        // addFields cannot be called client-side
        test.throws(function() {
            AccountsUsers.addFields('');
        }, function(err) {
            if (err instanceof Meteor.Error && err.details === 'AccountsUsers.addFields can be called server-side only!')
                return true;
        });
    } else {
